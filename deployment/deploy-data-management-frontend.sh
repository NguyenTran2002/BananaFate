#!/bin/bash
# Deploy data-management-frontend to Cloud Run
# Password-protected admin portal for managing banana data
...
echo "🚀 Deploying Data Management Frontend to Cloud Run"
echo "===================================================="
echo ""
...
# Define image name
IMAGE_NAME="${ARTIFACT_REGISTRY_URL}/${GCP_PROJECT_ID}/${ARTIFACT_REGISTRY_REPO}/${DATA_MANAGEMENT_FRONTEND_SERVICE}:latest"

# Navigate to frontend directory
FRONTEND_DIR="$PROJECT_ROOT/data-management/data-management-frontend"
...
gcloud run deploy "$DATA_MANAGEMENT_FRONTEND_SERVICE" \
  --image="$IMAGE_NAME" \
  --platform=managed \
  --region="$GCP_REGION" \
...
# Get service URL
DEPLOYED_FRONTEND_URL=$(gcloud run services describe "$DATA_MANAGEMENT_FRONTEND_SERVICE" \
  --region="$GCP_REGION" \
  --format='value(status.url)')

log_success "Data Management Frontend deployed successfully!"
...
echo ""
log_success "Data Management Frontend deployment complete!"
log_info "Login with password: $ADMIN_PASSWORD"
echo ""
